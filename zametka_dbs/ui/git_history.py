...
class _GitWorker(QProcess):
    ...
    def _on_finished(self, exit_code: int, exit_status: QProcess.ExitStatus) -> None:
        out: str = bytes(self._stdout).decode("utf-8", errors="replace")
        err: str = bytes(self._stderr).decode("utf-8", errors="replace")
...
class GitHistoryWidget(QWidget):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._diff_view: QTextEdit = QTextEdit()
    ...
    def _build_action_row(self) -> QWidget:
        row: QWidget = QWidget()
        action_layout: QHBoxLayout = QHBoxLayout(row)
        action_layout.setContentsMargins(8, 4, 8, 4)
        action_layout.setSpacing(4)

        self._stage_btn: QPushButton = QPushButton("Stage")
        ...
        self._unstage_btn: QPushButton = QPushButton("Unstage")
    ...
    def _is_git_repo(self) -> bool:
        if not self._vault_path:
            return False
        git_dir: str = os.path.join(self._vault_path, ".git")
    ...
    def _refresh(self) -> None:
        ...
        self._run_git(["log", "--oneline", "--abbrev-commit",
    ...
    def _on_log_ready(self, stdout: str) -> None:
        ...
        for sha, author, rel_date, subject in self._commit_cache:
            item: QListWidgetItem = QListWidgetItem(f"{sha}  {subject}")
            item.setData(Qt.ItemDataRole.UserRole, sha)
            item.setToolTip(f"{author} - {rel_date}")
            self._list.addItem(item)

    def _on_change_clicked(self, item: QListWidgetItem) -> None:
        filepath: str = item.data(Qt.ItemDataRole.UserRole)
        status: str = item.data(Qt.ItemDataRole.UserRole + 1)
        if not filepath:
            return
        is_staged: bool = status[0] != " " and status[0] != "?"
        ...
        if "?" in status:
            full_path: str = os.path.join(self._vault_path, filepath)
            ...
            args: list[str] = (["diff", "--cached", "--", filepath] if is_staged
                               else ["diff", "--", filepath])
            self._run_git(args, lambda out: self._on_diff_ready(out))

    def _on_diff_ready(self, stdout: str) -> None:
        self._diff_view.setPlainText(stdout[:5000] or "No diff")
        self._diff_view.setVisible(True)

    def _stage_selected(self) -> None:
        item: QListWidgetItem | None = self._changes_list.currentItem()
        if not item:
            return
        filepath: str = item.data(Qt.ItemDataRole.UserRole)
        if filepath:
            self._run_git(["add", filepath], lambda _: self._refresh())
    ...
    def _unstage_selected(self) -> None:
        ...
        filepath: str = item.data(Qt.ItemDataRole.UserRole)
        if filepath:
            self._run_git(["restore", "--staged", filepath], lambda _: self._refresh())
    ...
    def _on_commit_clicked(self, item: QListWidgetItem) -> None:
        ...
        self._action_row.setVisible(False)
        self._run_git(
            ["show", sha, "--stat", "--format=%H%n%an%n%ar%n%s%n%b"],
            lambda out: self._on_show_ready(out),
        )

    def _on_show_ready(self, stdout: str) -> None:
